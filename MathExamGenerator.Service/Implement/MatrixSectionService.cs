...
namespace MathExamGenerator.Service.Implement
{
    public class MatrixSectionService : BaseService<MatrixSectionService>, IMatrixSectionService
    {
        ...
        public async Task<BaseResponse<bool>> UpdateSection(Guid id, UpdateMatrixSectionRequest request)
        {
            var section = await _unitOfWork.GetRepository<MatrixSection>().SingleOrDefaultAsync(
                predicate: x => x.Id == id && x.IsActive == true);
            ...
            if (examUsed != null)
            {
                ...
                {
                    Status = StatusCodes.Status400BadRequest.ToString(),
                    Message = "Không thể cập nhật section vì ma trận đã được sử dụng để tạo đề thi.",
                    Data = false
                };
            }

            if (request.TotalQuestions.HasValue && request.TotalQuestions <= 0)
            {
                return new BaseResponse<bool>
                {
                    Status = StatusCodes.Status400BadRequest.ToString(),
                    Message = "TotalQuestions phải lớn hơn 0 nếu được cung cấp.",
                    Data = false
                };
            }

            if (request.TotalScore.HasValue && request.TotalScore <= 0)
            {
                return new BaseResponse<bool>
                {
                    Status = StatusCodes.Status400BadRequest.ToString(),
                    Message = "TotalScore phải lớn hơn 0 nếu được cung cấp.",
                    Data = false
                };
            }

            section.SectionName = request.SectionName ?? section.SectionName;
            section.TotalQuestions = request.TotalQuestions ?? section.TotalQuestions;
            section.TotalScore = request.TotalScore ?? section.TotalScore;
            section.UpdateAt = TimeUtil.GetCurrentSEATime();
